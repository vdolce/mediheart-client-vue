<template>
  ...
  >      
    <v-list-item three-line>
      <v-list-item-content>            
        <v-list-item-title class="custom-title">{{ $t('PersonalInfoModify.title') }}</v-list-item-title>            
        <v-list-item-subtitle class=" mb-4">{{ $t('PersonalInfoModify.subtitle') }}</v-list-item-subtitle>            
      </v-list-item-content>          
    </v-list-item>
    ...
    <v-card-actions class="mb-2">
      ...
      <v-btn color="secondary" class="mr-2" @click="$store.dispatch('updatePersonalInfo')">
        {{ $t('Common.confirm') }}
      </v-btn>
    </v-card-actions>
...
export default {
  ...
  data () {   
    ...
    return{          
      ...
      nameRules: [
        ...
        v => /^[aA-zZàèéìòù ']{2,30}$/.test(v) || this.$t('PersonalInfoModify.rules.onlyLetters'),
      ],
    }
  },
  ...
  computed:{
    form() {
      ...
      if(formData.personalInfo.birthDate == "" || formData.personalInfo.birthDate == undefined)
