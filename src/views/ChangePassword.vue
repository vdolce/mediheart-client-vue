<template>
  ...
  >
    <v-list-item three-line>
      <v-list-item-content>            
        <v-list-item-title class="custom-title">{{ $t('ChangePassword.title') }}</v-list-item-title>            
      </v-list-item-content>          
    </v-list-item>
    ...
    <v-card-actions class="mb-2">
      ...
      <v-btn color="secondary" class="mr-2" @click="$store.dispatch('confirmChangePassword')">
        {{ $t('Common.confirm') }}
      </v-btn>
    </v-card-actions>
  ...
  data () {   
    ...
    return{
      ...
      confirmationPasswordRules:[
        v => {       
         return v === this.form.newPassword1 || this.$t('ChangePassword.rules.confirmationPassword')
        },
      ],
